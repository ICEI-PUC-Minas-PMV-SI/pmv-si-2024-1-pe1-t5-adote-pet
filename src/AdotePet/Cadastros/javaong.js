function togglePasswordVisibility(btnSelector, inputSelector) {
  ...
  document.querySelector('#nomecompleto').addEventListener('keyup', () => {
    ...
    if (nomecompleto.value.length <= 10) {
      ...
      labelNome.innerHTML = '*Insira o nome da ONG';
      validNome = false;
    } else {
      labelNome.setAttribute('style', 'color: #3399BB');
  ...
  function cadastrar() {
    ...
    let listaUser = JSON.parse(localStorage.getItem('listaUser3') || '[]')
    ...
    localStorage.setItem('listaUser3', JSON.stringify(listaUser))


      msgSuccess.setAttribute('style', 'display: block')
      msgSuccess.innerHTML = '<strong>ONG Cadastrada!</strong>'
